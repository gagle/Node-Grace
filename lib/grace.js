...
(function (){
	...
	//Capture the GRACE_DESTROY message
	var on = process.on;
...
var clearTimers = function (){
	//Some timers could prevent the exit and the user may have forgotten to clear
	var p;
	for (p in timeouts){
		clearTimeout (timeouts[p]);
	}
	for (p in intervals){
...
(function (){
	...
	setTimeout = function (){
		var id = st.apply (undefined, arguments);
		var i = timeoutId++;
		id._graceClear = function (){
			delete timeouts[i];
		};
		timeouts[i] = id;
		return id;
	...
	setInterval = function (){
		var id = si.apply (undefined, arguments);
		var i = intervalId++;
		id._graceClear = function (){
			delete intervals[i];
		};
	...
	clearTimeout = function (timer){
		timer && timer._graceClear && timer._graceClear ();
		return ct.apply (undefined, arguments);
	};
	clearInterval = function (timer){
		timer && timer._graceClear && timer._graceClear ();
		return ci.apply (undefined, arguments);
	};
})();
...
Grace.prototype._overrideDestroy = function (){
	...
	cluster.Worker.prototype.destroy = function (){
		...
		this.process._handle.onexit = function (exitCode, signalCode){
			...
			//is overridden and signalCode is not null then it is safe to consider
...
Grace.prototype._callErrorHandler = function (error, req, res, cb){
	...
	if (this._defaultErrorHandlerCalled){
...
Grace.prototype.errorHandler = function (cb){
	//First Express middleware for handling uncaught exceptions thrown by a
	//request
	var me = this;
