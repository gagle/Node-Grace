...
Grace.prototype._callErrorHandler = function (error, req, res, cb, extraArgs){
	...
	var args = [error, req, res, preventDefault];
	if (extraArgs){
	...
	if (emitDefault){
		args.splice (3, 1);
		args.unshift ("error");
		//args == ["error", error, req, res, <extra_args...>]
...
Grace.prototype.errorHandler = function (cb){
	...
	return function (req, res, next){
		...
		d.on ("error", function (error){
			me._callErrorHandler (cleanError (error), req, res, cb);
			//dispose() is really bad, hangs up everything
...
Grace.prototype.redirectError = function (){
	var args = Array.prototype.slice.call (arguments);
	
	//Normal use
	//e.use (function (error, req, res, next){
	//	g.redirectError (error, req, res, <extra_args...>);
	//});
	if (args.length >= 3 && args[1] instanceof http.IncomingMessage &&
			args[2] instanceof http.ServerResponse){
		var error = args[0];
		var req = args[1];
		var res = args[2];
		args.splice (0, 3);
		this._callErrorHandler (cleanError (error), req, res,
				req._grace.errorHandler, args);
		return;
	}
	
	//Express shorthand
	//e.use (g.redirectError (<extra_args...>));
	var me = this;
	return function (error, req, res, next){
		me._callErrorHandler (cleanError (error), req, res, req._grace.errorHandler,
				args);
	};
};
